        ...
        <div class="mb-6">
            <form action="{{ route('users.index') }}" method="GET" class="space-y-3">
                ...
                <div class="flex flex-wrap gap-2 items-center justify-end">
                    ...
                    <button type="button"
                        data-export-btn
                        data-excel-url="{{ route('users.export', request()->only(['search', 'role', 'status', 'date_from', 'date_to'])) }}"
                        data-pdf-url="{{ route('users.export', array_merge(request()->only(['search', 'role', 'status', 'date_from', 'date_to']), ['format' => 'pdf'])) }}"
