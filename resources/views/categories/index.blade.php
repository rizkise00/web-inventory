...
<div class="min-h-screen py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Search and Actions -->
        <div class="mb-6">
            <form action="{{ route('categories.index') }}" method="GET" class="space-y-3">
                <!-- Filter fields: 2 equal-width columns -->
                <div class="flex flex-col sm:flex-row gap-3">
                    ...
                    <div class="relative flex-1">
                        <input type="hidden" name="date_from" id="date_from_categories" value="{{ request('date_from') }}">
                        <input type="hidden" name="date_to" id="date_to_categories" value="{{ request('date_to') }}">
                        <input type="text" id="daterange_categories" placeholder="Select date range..." class="w-full py-2 px-3 pl-9 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm transition-all text-gray-700 cursor-pointer bg-white" readonly>
                        <div class="absolute left-3 top-2.5 text-gray-400">
                ...
                <div class="flex flex-wrap gap-2 items-center justify-end">
                    ...
                    <a href="{{ route('categories.index') }}" class="bg-amber-500 hover:bg-amber-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:shadow-lg text-sm transition-all duration-200">Reset</a>
                    @endif
                    <a href="{{ route('categories.export', request()->only(['search', 'date_from', 'date_to'])) }}" class="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-semibold py-2 px-5 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center text-sm">
...
(function() {
    const fp = flatpickr('#daterange_categories', {
        mode: 'range',
        dateFormat: 'Y-m-d',
        onChange: function(selectedDates) {
            document.getElementById('date_from_categories').value = selectedDates.length >= 1 ? flatpickr.formatDate(selectedDates[0], 'Y-m-d') : '';
            document.getElementById('date_to_categories').value   = selectedDates.length >= 2 ? flatpickr.formatDate(selectedDates[1], 'Y-m-d') : '';
        }
    });
    const df = document.getElementById('date_from_categories').value;
    const dt = document.getElementById('date_to_categories').value;
