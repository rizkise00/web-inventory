        ...
        <div class="mb-6">
            <form action="{{ route('items.index') }}" method="GET" class="space-y-3">
                ...
                <div class="flex flex-wrap gap-2 items-center justify-end">
                    ...
                    <button type="button"
                        data-export-btn
                        data-excel-url="{{ route('items.export', request()->only(['search', 'low_stock', 'date_from', 'date_to'])) }}"
                        data-pdf-url="{{ route('items.export', array_merge(request()->only(['search', 'low_stock', 'date_from', 'date_to']), ['format' => 'pdf'])) }}"
